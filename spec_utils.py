...
def build_mask(time, flux, fixed_params, mask_buffer=50):
    ...
    u1, u2, per, r, t0, a, V, C, Ls, Lc, S, w0 = fixed_params
    per, r, t0, a, inc, e, w = canon_from_eastman(*fixed_params[2:-1])
...
def build_logp(
    ...
    def log_prob_vary_w0(p):
        ...
        try:
            ...
            mu, jac = reparam(time, [u1, u2, per, r, t0, a, V, C, Ls, Lc, S])
            mu = mu * f + trend
    ...
    def log_prob_gp(p):
        ...
        try:
            ...
            pr = u1_prior.prior(u1)
            pr += u2_prior.prior(u2)
            pr += uniform_prior(0, 6).prior(lsigma)
